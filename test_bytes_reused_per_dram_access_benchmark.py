#!/usr/bin/env python3
"""
SAXPY benchmark: bytes reused per DRAM access (DCache vs SMEM).
...
Sweep variable:
- num_passes over the same vector footprint.

Metric:
- bytes_reused_per_dram_access = reused_bytes / dram_transactions
- reused_bytes = (global_requests - dram_transactions) * WORD_BYTES
  where global_requests counts only global-memory requests.

Outputs:
- bytes_reused_per_dram_access.csv
- bytes_reused_per_dram_access_report.txt
- bytes_reused_per_dram_access_plot.png

Run:
    ./.venv/bin/python test_bytes_reused_per_dram_access_benchmark.py
"""
...
CSV_PATH = ROOT / "bytes_reused_per_dram_access.csv"
REPORT_PATH = ROOT / "bytes_reused_per_dram_access_report.txt"
PLOT_PATH = ROOT / "bytes_reused_per_dram_access_plot.png"
...
class ReuseRow:
    ...
    dcache_reused_bytes: int
    dcache_bytes_reused_per_dram_access: float
    smem_global_requests: int
    smem_dram_transactions: int
    smem_reused_bytes: int
    smem_bytes_reused_per_dram_access: float
    dcache_cycles: int
    smem_cycles: int
...
def _run_dcache_saxpy_stream(
    *,
    ...
    behind = DCACHE["LatchIF"](name="saxpy_reuse_lsu_to_dcache")
    mem_req_if = DCACHE["LatchIF"](name="saxpy_reuse_dcache_to_mem")
    mem_resp_if = DCACHE["LatchIF"](name="saxpy_reuse_mem_to_dcache")
    response_if = DCACHE["ForwardingIF"](name="saxpy_reuse_dcache_to_lsu")

    stage = DCACHE["LockupFreeCacheStage"](
        name="SaxpyReuseDCache",
        behind_latch=behind,
...
def run_benchmark() -> List[ReuseRow]:
    rows: List[ReuseRow] = []

    for num_passes in PASS_SWEEP:
        ...
        dcache_reused_bytes = int(
            max(0, stream.dcache_global_requests - dcache_dram_txn) * WORD_BYTES
        )
        smem_reused_bytes = int(
            max(0, stream.smem_global_requests - smem_dram_txn) * WORD_BYTES
        )
        ...
        rows.append(
            ReuseRow(
                ...
                dcache_reused_bytes=dcache_reused_bytes,
                dcache_bytes_reused_per_dram_access=float(dcache_reused_bytes)
                / float(max(dcache_dram_txn, 1)),
                smem_global_requests=int(stream.smem_global_requests),
                smem_dram_transactions=int(smem_dram_txn),
                smem_reused_bytes=smem_reused_bytes,
                smem_bytes_reused_per_dram_access=float(smem_reused_bytes)
                / float(max(smem_dram_txn, 1)),
...
def _write_csv(rows: Iterable[ReuseRow], path: Path) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                ...
                "dcache_reused_bytes",
                "dcache_bytes_reused_per_dram_access",
                "smem_global_requests",
                "smem_dram_transactions",
                "smem_reused_bytes",
                "smem_bytes_reused_per_dram_access",
                "dcache_cycles",
                "smem_cycles",
            ]
        )
        for row in rows:
            writer.writerow(
                [
                    ...
                    row.dcache_reused_bytes,
                    f"{row.dcache_bytes_reused_per_dram_access:.8f}",
                    row.smem_global_requests,
                    row.smem_dram_transactions,
                    row.smem_reused_bytes,
                    f"{row.smem_bytes_reused_per_dram_access:.8f}",
                    row.dcache_cycles,
                    row.smem_cycles,
                ]
            )
...
def _build_table(rows: Iterable[ReuseRow]) -> str:
    lines = [
        f"{'Pass':>4} | {'D$ Reuse/DRAM(B)':>16} | {'SMEM Reuse/DRAM(B)':>19} | {'D$ DRAM':>7} | {'SMEM DRAM':>9} | {'D$ Cyc':>6} | {'SMEM Cyc':>8}"
    ]
    lines.append("-" * 88)
    for row in rows:
        lines.append(
            f"{row.num_passes:>4} | "
            f"{row.dcache_bytes_reused_per_dram_access:>16.3f} | "
            f"{row.smem_bytes_reused_per_dram_access:>19.3f} | "
...
def _write_report(rows: List[ReuseRow], path: Path) -> None:
    first = rows[0]
    last = rows[-1]
    sections = [
        "SAXPY: Bytes Reused per DRAM Access (DCache vs SMEM)",
        "====================================================",
        "",
        "Workload",
        "--------",
        f"- Full SAXPY stream over {TOTAL_ELEMENTS} elements",
        f"- Fixed x reuse window: {X_REUSE_WORDS} words",
        ...
        "Edge Summary",
        "------------",
        f"- {first.num_passes} pass: D$ reuse/DRAM={first.dcache_bytes_reused_per_dram_access:.3f} B, SMEM reuse/DRAM={first.smem_bytes_reused_per_dram_access:.3f} B",
        f"- {last.num_passes} passes: D$ reuse/DRAM={last.dcache_bytes_reused_per_dram_access:.3f} B, SMEM reuse/DRAM={last.smem_bytes_reused_per_dram_access:.3f} B",
        "",
        _build_table(rows),
        "",
        f"CSV: {CSV_PATH.name}",
...
def _plot_rows(rows: List[ReuseRow], path: Path) -> None:
    try:
        import matplotlib
    ...
    xs = [row.num_passes for row in rows]
    dcache_reuse = [row.dcache_bytes_reused_per_dram_access for row in rows]
    smem_reuse = [row.smem_bytes_reused_per_dram_access for row in rows]
    ...
    ax.plot(
        ...
        marker="o",
        markersize=5.0,
        label="DCache bytes reused / DRAM access",
    )
    ax.plot(
        xs,
        smem_reuse,
        ...
        marker="s",
        markersize=4.8,
        label="SMEM bytes reused / DRAM access",
    )
    ax.fill_between(xs, dcache_reuse, color="#c05621", alpha=0.08)
    ax.fill_between(xs, smem_reuse, color="#1f6f8b", alpha=0.08)
    ...
    ax.set_xlabel("SAXPY Pass Count Over Same Footprint", color="#2f2a24")
    ax.set_ylabel("Bytes Reused per DRAM Access", color="#2f2a24")
    ax.legend(loc="upper left", frameon=False)

    fig.suptitle(
        "SAXPY Reuse Efficiency: DCache vs SMEM",
        fontsize=16,
        weight="bold",
        color="#2f2a24",
        y=0.96,
    ...
    fig.text(
        0.5,
        0.03,
        "Same SAXPY stream, same pass sweep. Higher reused bytes per DRAM access means stronger caching reuse.",
        ha="center",
        fontsize=9.3,
        color="#5c5348",
    )
...
def _print_summary(rows: List[ReuseRow]) -> None:
    print("SAXPY Bytes Reused per DRAM Access (DCache vs SMEM)")
    print("---------------------------------------------------")
